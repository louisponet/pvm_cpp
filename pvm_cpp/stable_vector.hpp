#ifndef stable_vector_hpp_INCLUDED
#define stable_vector_hpp_INCLUDED
// Based on stable_vector from david-grs (https://github.com/david-grs/stable_vector/blob/master/stable_vector.h)
...
class stable_vector {
   ...
   private:
    ...
    static_assert(is_pow2<ChunkSize>::value,
                  "ChunkSize needs to be a power of 2");
   ...
   public:
    ...
    struct const_iterator
        : public iterator_base<__const_self>,
          public boost::random_access_iterator_helper<const_iterator,
                                                      const value_type> {
    ...
    stable_vector(const stable_vector& other);      // copy
    stable_vector(stable_vector&& other) noexcept;  // move
   ...
   private:
    ...
    storage_type m_chunks;  //ptrs to chunks which themselves are static vectors
};
...
template <class T, std::size_t ChunkSize>
stable_vector<T, ChunkSize>& stable_vector<T, ChunkSize>::operator=(
    stable_vector v) {
    swap(v);
    return *this;
}
...
template <class T, std::size_t ChunkSize>
typename stable_vector<T, ChunkSize>::chunk_type&
stable_vector<T, ChunkSize>::last_chunk() {
    if (likely_false(m_chunks.empty() ||
                     m_chunks.back()->size() == ChunkSize)) {
        add_chunk();
    }
...
void stable_vector<T, ChunkSize>::reserve(size_type new_capacity) {
    ...
    for (difference_type i = new_capacity - initial_capacity; i > 0;
         i -= ChunkSize) {
        add_chunk();
    }
}
...
template <class T, std::size_t ChunkSize>
typename stable_vector<T, ChunkSize>::reference stable_vector<T, ChunkSize>::at(
    size_type i) {
...
#endif  // stable_vector_hpp_INCLUDED
