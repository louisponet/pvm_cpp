...
boost::filesystem::path Client::portpath() {
...
void Client::connect() {
    ...
    boost::filesystem::path pp = portpath();
...
auto Client::request(const std::string& method, const std::string& path, const std::string& errmsg,
                     Func&& f) -> std::invoke_result_t<Func, httplib::Result&> {
    httplib::Request req;
    req.method = method;
    req.path   = path;
    if (!cli)
...
auto Client::request(const std::string& method, const std::string& path, const Data& data,
...
void Client::load_data(const std::string& name, const std::string& path) {
    ...
    request("POST", fmt::format("/data/load/{}/{}", name, path), "Error loading data",
            [](auto& res) {
                if (res->status != 200)
...
void Client::load_data(const std::string& name) {
    // TODO make this ispath on the server
    request("POST", fmt::format("/data/load/{}", name), "Error loading data", [](auto& res) {
        if (res->status != 200)
            throw std::invalid_argument("Server error loading data");
    });
}
...
void Client::unload_data(const std::string& name) {
    // TODO make this ispath on the server
    request("POST", fmt::format("/data/unload/{}", name), "Error unloading data", [](auto& res) {
        if (res->status != 200)
...
std::vector<std::string> Client::list_data() {
    // TODO make this ispath on the server
    return request("GET", fmt::format("/data/list"), "Error listing data", [](auto& res) {
        if (res->status != 200)
...
void Client::init_plugin(const std::string& plugin_name, const std::string& dataset_name) {
    // TODO make this ispath on the server
    request("POST", fmt::format("/plugin/init/{}/{}", plugin_name, dataset_name),
...
void Client::run_plugin(const std::string& plugin_name, const std::string& dataset_name) {
    // TODO make this ispath on the server
...
void Client::finalize_plugin(const std::string& plugin_name, const std::string& dataset_name) {
    // TODO make this ispath on the server
    request("POST", fmt::format("/plugin/finalize/{}/{}", plugin_name, dataset_name),
...
void Client::add_bar(const std::string& name, const Bar& bar) {
    // TODO make this ispath on the server
    request("POST", fmt::format("/data/add/{}/", name), bar, "Error adding bar", [](auto& res) {
        if (res->status != 200)
