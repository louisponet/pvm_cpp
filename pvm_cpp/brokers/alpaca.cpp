...
AlpacaBroker::AlpacaBroker(const std::string alpaca_key_id_, const std::string alpaca_secret_,
    ...
    : cur_requests(0), max_requests(max_requests) {

    httplib::SSLClient client("paper-api.alpaca.markets");

    httplib::Result resp = client.Get("/v2/account", ::headers(alpaca_key_id_, alpaca_secret_));
...
std::vector<Bar> AlpacaBroker::get_bars(const std::string& ticker, const std::string& start,
                                        const std::string& stop, const std::string& timeframe) {
    using namespace nlohmann;

    httplib::SSLClient client("data.alpaca.markets");

    httplib::Params params{
        {"limit", "1000"}, {"timeframe", timeframe}, {"start", start}, {"end", stop}};

    std::string query_string = httplib::detail::params_to_query_str(params);

    httplib::Result resp =
        client.Get(fmt::format("/v2/stocks/{}/bars?{}", ticker, query_string).c_str(), headers());
...
std::vector<Bar> AlpacaBroker::parse_bars(httplib::Result& resp) {
    json data = nlohmann::json::parse(resp->body);
    ...
    for (int i = 0; i < nbars; i++) {
        json::object_t curbar = datbars[i];
